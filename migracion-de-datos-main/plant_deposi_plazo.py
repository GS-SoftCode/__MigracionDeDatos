...
for index, row in df.iterrows():
    ...
    data = (row['cod_producto'], row['cod_cuenta'], row['cod_socio'], row['num_documento'], row['val_deposito'], row['val_efectivo'], row['val_cheques'], row['val_tasa_interes'], row['val_impuesto'], sts_deposito[:2], fec_deposito, row['num_plazo'], row['num_cuotas_pago_interes'], fec_vencimiento, row['txt_referencia'], row['nom_beneficiario'], row['ape_beneficiario'], cod_tipo_char, row['num_id_ben'], row['cod_usrmod'],fec_usrmod, row['val_interes'], row['val_tasa_impuesto'], row['cod_oficina'], row['cod_cuenta_contable'], row['cod_producto_socio'], row['cod_cuenta_socio'], sts_forma_char, row['val_saldo_impuesto'], row['val_saldo_deposito'], row['val_saldo_interes'], row['val_tir'], row['val_tea'])
